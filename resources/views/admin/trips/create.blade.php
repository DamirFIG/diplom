...
<div class="admin-page">
    ...
    <form action="{{ route('admin.trips.store') }}" method="POST" enctype="multipart/form-data" class="admin-form">
        @csrf
        <input type="hidden" name="type" value="route">

        <div class="form-group">
            <label for="title">Название поездки *</label>
            <input type="text" name="title" id="title" value="{{ old('title') }}" required placeholder="Например: Водная прогулка по Неве">
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select name="activity_type" id="activity_type" required>
                    ...
                    <option value="гидроцикл" {{ old('activity_type') === 'гидроцикл' ? 'selected' : '' }}>Гидроцикл</option>
                    <option value="банан" {{ old('activity_type') === 'банан' ? 'selected' : '' }}>Банан</option>
                    <option value="флайборд" {{ old('activity_type') === 'флайборд' ? 'selected' : '' }}>Флайборд</option>
                    <option value="сапборд" {{ old('activity_type') === 'сапборд' ? 'selected' : '' }}>Сапборд</option>
                    <option value="катамаран" {{ old('activity_type') === 'катамаран' ? 'selected' : '' }}>Катамаран</option>
                </select>
            </div>

            <div class="form-group">
                <label for="price">Цена (₽) *</label>
                <input type="number" name="price" id="price" value="{{ old('price') }}" required min="0" placeholder="5000">
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" name="max_people" id="max_people" value="{{ old('max_people') }}" min="1" placeholder="2">
            </div>

            <div class="form-group">
                <label for="duration_minutes">Длительность (мин)</label>
                <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes') }}" min="1" placeholder="60">
            </div>

            <div class="form-group">
                <label for="min_age">Мин. возраст</label>
                <input type="number" name="min_age" id="min_age" value="{{ old('min_age') }}" min="1" placeholder="16">
            </div>
        </div>

        <div class="form-group">
            <label for="description">Описание</label>
            <textarea name="description" id="description" rows="4" placeholder="Описание поездки...">{{ old('description') }}</textarea>
        </div>

        <div class="form-group">
            <label for="gallery">Фотографии *</label>
            <input type="file" name="gallery[]" id="gallery" multiple accept="image/*" required>
                ...
                <div class="route-points-list" id="route-points-list">
                    ...
                    <div class="route-point-item start" data-index="0" data-type="start">
                        ...
                        <div class="point-body">
                            <div class="point-row">
                                <input type="text" name="route_points[0][title]" placeholder="Название точки" value="{{ old('route_points.0.title') }}" required>
                                <input type="text" name="route_points[0][lat]" class="point-lat" placeholder="Широта" value="{{ old('route_points.0.lat') }}" readonly required>
                                <input type="text" name="route_points[0][lng]" class="point-lng" placeholder="Долгота" value="{{ old('route_points.0.lng') }}" readonly required>
                            </div>
                            <div class="point-row">
                                <input type="text" name="route_points[0][description]" placeholder="Описание" value="{{ old('route_points.0.description') }}">
                                <input type="file" name="route_points[0][image_file]" accept="image/*" placeholder="Фото точки">
                            </div>
        ...
        <div class="form-actions">
            <button type="submit" class="btn-submit">Создать поездку</button>
